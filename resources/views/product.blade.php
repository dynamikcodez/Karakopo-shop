...
@section('meta_image', $product->primaryImage ? asset('storage/' . $product->primaryImage->image_path) : asset('images/logo.png'))
        ...
        <div class="mb-8 lg:mb-0">
            <div class="aspect-square rounded-2xl bg-white overflow-hidden mb-4 border border-gray-100 shadow-sm relative">
                @if($product->images->count() > 0)
                    ...
                    <img src="{{ asset('storage/' . $primary->image_path) }}" id="main-image" class="w-full h-full object-cover transition-all duration-300" alt="{{ $product->name }}">
                @else
            ...
            <div class="grid grid-cols-4 sm:grid-cols-5 gap-3">
                @foreach($product->images as $image)
                    <button type="button" class="aspect-square rounded-xl bg-white overflow-hidden border-2 border-gray-200 hover:border-maroon focus:outline-none focus:border-maroon transition-all shadow-sm" onclick="document.getElementById('main-image').src='{{ asset('storage/' . $image->image_path) }}'">
                        <img src="{{ asset('storage/' . $image->image_path) }}" class="w-full h-full object-cover" alt="Thumbnail">
                    </button>
                @endforeach
            </div>
            @endif
